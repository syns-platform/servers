...
// 
// @param gc *gin.Context
func (pc *PostController) ReactComment(gc *gin.Context) {
	// @notice reaction.Post_ID = comment.Comment_ID in this case
	...
	// sanitizing param.post_id (a.k.a commentId)
	idMatched, idErr := regexp.MatchString(`^[a-fA-f0-9]{24}$`, param.Post_ID.Hex())
	if idErr != nil {gc.AbortWithStatusJSON(400, gin.H{"error": "!REGEX - cannot test param.Post_ID using regex"}); return;}
	if !idMatched {gc.AbortWithStatusJSON(400, gin.H{"error": "!TIERID - param.Post_ID is not valid"}); return;}
	...
	// invoke PostDao.ReactComment()
	if err := pc.PostDao.ReactComment(param); err != nil {gc.AbortWithStatusJSON(500, gin.H{"error": err.Error()}); return;}
...
// 
// @param gc *gin.Context
func (pc *PostController) UpdateReplyContent(gc *gin.Context) {
	// declare param holder
	param := &models.Reply{}
	...
	// invoke PostDao.UpdateReplyContent
	if err := pc.PostDao.UpdateReplyContent(param); err != nil {gc.AbortWithStatusJSON(500, gin.H{"error": err.Error()}); return;}
	
	// http response
	gc.JSON(200, "Reply content successfully updated")
}
...
// 
// @param gc *gin.Context
func (pc *PostController) ReactReply(gc *gin.Context) {
	...
	// invoke PostDao.ReactReply
	if err := pc.PostDao.ReactReply(param); err != nil {gc.AbortWithStatusJSON(500, gin.H{"error": err.Error()}); return;}
